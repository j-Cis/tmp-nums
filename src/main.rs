...
fn main() {
    ...
    let potega = Node::Pow(Box::new(k.clone()), Box::new(n.clone()));
    ...
    // Bezpiecznie klonujemy, bo to ostatnia operacja w pliku
    let cb_negatywne = Node::Mul(Box::new(Node::Num(-1)), Box::new(Node::Mul(Box::new(c.clone()), Box::new(b.clone()))));
    let ad_odejmowanie = Node::Mul(Box::new(a.clone()), Box::new(d.clone()));
    let gora_odejmowanie = Node::Add(Box::new(ad_odejmowanie), Box::new(cb_negatywne));
    let dol_odejmowanie = Node::Mul(Box::new(b.clone()), Box::new(d.clone()));
